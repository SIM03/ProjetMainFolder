   ...
   public class DynamicPhysicalObject : PhysicalObject
   {
      ...
      public bool Selected
      {
         ...
         set
         {
            if (value)
            {
               GameCamera = Game.Services.GetService(typeof(CaméraSubjective)) as CaméraSubjective;
            }
            selected_ = value;
         }
      }
      ...
      private void FollowCamera()
      {
         ...
         //Monde *= Matrix.CreateTranslation((new Vector3(GameCamera.Direction.X, 0, GameCamera.Direction.Z * GameCamera.DéplacementDirection)) /*+ (GameCamera.Latéral * GameCamera.DéplacementLatéral)*/);
         //Monde *= Matrix.CreateTranslation(Position);
      ...
      private bool VérifierBoîteDeCollision(Vector3 anciennePosition, Vector3 rotation)
      {
         bool CollisionEnVue = false;
